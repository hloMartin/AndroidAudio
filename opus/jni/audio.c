...
#include <math.h>
#include "utils.h"
#include "com_martin_audiolib_OpusTool.h"
...
JNIEXPORT jint Java_com_martin_audiolib_OpusTool_startRecord(JNIEnv *env, jclass class, jstring path) {
...
JNIEXPORT jint Java_com_martin_audiolib_OpusTool_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame(frameBytes, len);
}

JNIEXPORT void Java_com_martin_audiolib_OpusTool_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}
...
int seekPlayer(float position) {
    LOGE("op_pcm_seek failed: %f", position);
    if (!_opusFile || !_isSeekable || position < 0) {
        LOGE("_opusFile:%s  isSeekAble:%d", _opusFile, _isSeekable);
        return 0;
    }
    ...
    LOGE("op_pcm_seek current position:%d", _currentPcmOffset);
...
int initPlayer(const char *path) {
    ...
    LOGE("op_open_file totalPcmDuration: %f  |   %d", _totalPcmDuration, _totalPcmDuration);
...
JNIEXPORT jlong Java_com_martin_audiolib_OpusTool_getTotalPcmDuration(JNIEnv *env, jclass class) {
    //LOGE("getTotalPcmDuration:%f  | %d", _totalPcmDuration, _totalPcmDuration);
    return _totalPcmDuration;
}

JNIEXPORT void Java_com_martin_audiolib_OpusTool_readOpusFile(JNIEnv *env, jclass class, jobject buffer, jint capacity, jintArray args) {
...
JNIEXPORT jint Java_com_martin_audiolib_OpusTool_seekOpusFile(JNIEnv *env, jclass class, jfloat position) {
    return seekPlayer(position);
}

JNIEXPORT jint Java_com_martin_audiolib_OpusTool_openOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT void Java_com_martin_audiolib_OpusTool_closeOpusFile(JNIEnv *env, jclass class) {
    cleanupPlayer();
}

JNIEXPORT jint Java_com_martin_audiolib_OpusTool_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    ...
    if (file != NULL) {
        ...
        LOGE("[isOpusFile] error:%d", error);
        op_free(file);
        
        result = error == OPUS_OK;
    }
    LOGE("[isOpusFile] result:%d", result);
...
JNIEXPORT jbyteArray Java_com_martin_audiolib_OpusTool_getWaveform2(JNIEnv *env, jclass class, jshortArray array, jint length) {
...
JNIEXPORT jbyteArray Java_com_martin_audiolib_OpusTool_getWaveform(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    jbyteArray result = 0;
