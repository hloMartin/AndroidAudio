...
static int op_open1(OggOpusFile *_of,
  ...
  *&_of->callbacks=*_cb;
  ...
  if(seekable){
    LOGE("[op_open1] _initial_bytes:%u", _initial_bytes);
    //opus_int64 pos;
    LOGE("[op_open1] 111 _initial_bytes:%u", _initial_bytes);
    if(OP_UNLIKELY(_of->callbacks.tell==NULL))return OP_EINVAL;
    LOGE("[op_open1] 222 _initial_bytes:%u", _initial_bytes);
    //opus_int64 pos=(*_of->callbacks.tell)(_of->stream);
    size_t pos=(*_of->callbacks.tell)(_of->stream);
    LOGE("[op_open1] 333 pos:%ld  _initial_bytes:%u", pos, _initial_bytes);
    /*If the current position is not equal to the initial bytes consumed,
       absolute seeking will not work.*/
    if(OP_UNLIKELY(pos!=(size_t)_initial_bytes))return OP_EINVAL;
  }
